...
public class DragSortListView extends ListView {
  ...
  private boolean mLongPressEnabled = false; // allow long press to initiate dragging the item
  ...
  public DragSortListView(Context context, AttributeSet attrs) {
    ...
    if (attrs != null) {
      ...
      mLongPressEnabled = a.getBoolean(
        R.styleable.DragSortListView_long_press_enabled, false);
  ...
  public boolean onInterceptTouchEvent(MotionEvent ev) {
    if (mGestureDetector == null) {
      if (mRemoveMode == FLING || mLongPressEnabled) {
        mGestureDetector = new GestureDetector(getContext(), new SimpleOnGestureListener() {
          @Override
          public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
              float velocityY) {
            if (mRemoveListener != null  && mRemoveMode == FLING && mFloatView != null) {
              if (velocityX > 1000) {
                Rect r = mTempRect;
